...
def write_excel():
    login_test = pd.read_excel("test_case_excel\\login_test_cases_checkbox.xlsx")
    for index, row in login_test.iterrows():
        if bool(row['checkbox']) == True:
    ...
    login_test.to_excel('test_case_excel\\login_test_cases_results_checkbox.xlsx')
    driver.close()
